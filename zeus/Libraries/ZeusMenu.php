...
class ZeusMenu
{
    ...
    public function create_menu($title,$segment=[],$icon,$route_name='',$route_prefix=[],$menu_parent=NULL,$group_access=[])
    {
        ...
        if($save)
        {
            if(!empty($group_access))
            {
                ...
                DB::table('zeus_menu_group')->insert($insert_group);
            }
            return array(
                'status'=>true,
    ...
    public function edit_menu($menu_id,$title,$segment=[],$icon,$route_name,$route_prefix=[], $group_access = [])
    {
        ...
        $save = $edit->save();
        if ($save) {
            DB::table('zeus_menu_group')->where('zeus_menu_id',$menu_info->id)->delete();
            if (!empty($group_access)) {
                $insert_group = [];
                foreach ($group_access as $ga) {
                    $insert_group[] = [
                        'zeus_menu_id' => $menu_info->id,
                        'user_group_id' => $ga
                    ];
                }
                DB::table('zeus_menu_group')->insert($insert_group);
    ...
    public function delete_menu($menu_id,$delete_child=FALSE)
    {
        ...
        if($delete)
        {
            if($delete_child == TRUE)
            {
                ...
                DB::table('zeus_menu_group')->where('zeus_menu_id',$menu_id)->delete();
                DB::table('zeus_menu_group')
                ->join('zeus_menu','zeus_menu_group.zeus_menu_id','zeus_menu.id')
                ->where('zeus_menu.zeus_menu_id', $menu_id)->delete();
            }else{
